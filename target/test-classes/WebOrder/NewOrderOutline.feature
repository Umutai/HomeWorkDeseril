Feature: Validation of New order using Scenario Outline
  ...
  Scenario Outline:  New Order Validation
    ...
    And the user enter address info "<name>", "<adress>", "<city>", "<state>", "<zipcode>"
    * the user enter payment info "<cardType>", "<cardNum>", "<expDate>"
    Then the user validate success message
    And the user validate new order details "<productName>","<quantity>","<name>","<adress>","<city>","<state>","<zipcode>","<cardType>", "<cardNum>", "<expDate>"
    Scenarios:
